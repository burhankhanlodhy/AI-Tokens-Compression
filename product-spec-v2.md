...
- **AC-P1a-gate (empty-box calibration — the spec carries the script, and the re-run carries the gate).** Before the corrected harness may spend a single provider dollar, the estimator must prove it reads ~0% when designed to read 0%: `benchmark/empty_box.py` (committed, deterministic seed 20260916, offline, sub-second) draws both arms from the SAME measured output-length distribution (CV=0.2415 from the on-file SD gate) and asserts the **null arm** (no effect) reports |estimate| ≤ 5pp with a 95% bootstrap CI **containing 0**, while the **positive-control arm** (true 15% effect) reports an estimate inside [12, 18]pp with a 95% CI **excluding 0**. A harness that reports savings on an empty box is broken, not lucky; a harness that cannot resolve a synthetic 15% effect is the audit's failure mode verbatim ("could not distinguish no effect from ~17% effect"). Exit code 0 = calibrated/cleared to run; exit code 1 = **block open — no re-run spend until green.** QA runs it as the first step of any P1-1 gate; the published headline must name which side of this gate it ran under.
