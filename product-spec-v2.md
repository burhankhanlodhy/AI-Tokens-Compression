...
- **AC-P1e. L1 clean target:** strip only lossless structural content — pretty-printed JSON whitespace, duplicated/empty system blocks, dead retrieval/RAG metadata (no-bearing-on-answer fields) — **with no change to answer content whatsoever**. Success bar: **≥15% input-token reduction on RAG/JSON-heavy categories** at **byte-identical answer semantics** (round-trip equivalence, no judge needed). Must be **deterministic** (stable cache prefix, defaults clean to the same bytes) so PA-4 caching is preserved.
