...
- **AC-P1a-gate (empty-box calibration — the spec carries the script, and the re-run carries the gate).** Before the corrected harness may spend a single provider dollar, the estimator must prove it reads ~0% when designed to read 0%. `benchmark/empty_box.py` **imports the production estimator by reference** (`run_benchmark.estimate` → shared `estimator.estimate`) — it re-implements nothing, so a patched/sabotaged estimator flips the gate's verdict (proved both directions: 99%-estimator → null FP 100% / control 0%; corrected-preview → measured rates below). Contract: over **N=200 seeded replications** (deterministic, offline, sub-second), the **null arm** (same measured output-length distribution both sides, CV=0.2415 from the on-file SD gate) must show a **false-positive rate ≤ 5%** (|estimate| > 5pp or 95% CI excluding 0), and the **positive-control arm** (true 15% effect) must show a **success rate ≥ 90%** (estimate in [12,18]pp with 95% CI excluding 0). A harness that reports savings on an empty box is broken, not lucky; a harness that cannot resolve a synthetic 15% effect is the audit's failure mode verbatim. Exit 0 = calibrated/cleared; exit 1 = **block open — no re-run spend until green.** QA runs it as the first step of any P1-1 gate; the published headline must name which side of this gate it ran under.
- **AC-P1a-gate measurement (2026-09, verified by PM — feeds C-4b):** at the measured CV and a realistic 40-prompt mix, **k = 5 per arm does NOT clear the contract even with the corrected ratio-of-sums + bootstrap math** (measured: null FP 6.5%, control success 84.5% — both fail PM's 5%/90%). The gate's power sweep: k=10 → control 90.5% but null 7.5% (fail); **k ≥ 12 clears both (null 5.0%, control 97.5%)**. The k=1 mean-of-ratios estimator currently in the shipped harness is catastrophically below contract (null FP 51.5%, control 31.5%). **C-4b implication: the corrected estimator must ship `HARNESS_K ≥ 12`** (or justify more prompts / a tighter designed CV with a fresh SD gate) **and pass this gate at its shipped k** — k=5 is not enough, and the gate is the arbiter. Do NOT weaken the 5%/90% contract to make any estimator pass; change the design (k, n_prompts, or re-measure CV) so the contract is met honestly.
