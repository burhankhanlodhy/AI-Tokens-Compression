...
Status: DRAFT for approval — do not start implementation until ratified. Rev 2: rebased onto post-gate tip fb6d849d; all main.py/schema line cites re-verified at that tip; AC-TQ6/7 given explicit `required`-mode preconditions; AC-TQ11 corrected (`.env.example` exists at token-saver/.env.example).
...
Grounded against branch tip `fb6d849d12b2fa69867029f8a2ec4ea38f240148` (task/t_373850f1 — the V1.2.2 post-gate tip: a28ba93cfed05783aee3e4f01a95ad6893e3d21f + the F-QA1 test fix; the intervening f52fb6f→a28ba93 diff was V1.2.2 tool-ledger work touching none of the cited mechanisms). file:line cites below were verified at that tip and are the shipped mechanisms, not aspirations. Implementation branches from this tip.
...
- **G-A. The data plane authenticates nobody.** `POST /v1/chat/completions` (main.py:605) and `POST /v1/embeddings` (main.py:1594) perform no proxy-key check. `Authorization`/`x-api-key` headers are the caller's **upstream BYOK credential**, extracted and forwarded verbatim (`_upstream_credential`, main.py:361-366). The `api_keys` table, its hash/last4/plaintext-once lifecycle, and the rotate/revoke endpoints (main.py:1720/1750/1771/1798) mint keys that currently gate **nothing** on the request path.
- **G-B. The ledger and caches are pinned to the default tenant.** Every ledger insert hardcodes `tenant_id = '00000000-…'` and never writes `api_key_id` (stats.py:249-271). Exact-prefix cache lookup/record hardcode the same tenant (caching.py:73-79, 95-97). The semantic cache is tenant-scoped internally but the request path passes `DEFAULT_TENANT_ID` unconditionally (main.py:843). Result: in any multi-key deployment, per-tenant spend is unmeasurable and one tenant's cached prefixes/entries are accounted under `default`.
- **G-C. No quota or rate-limit mechanism exists anywhere.** `spend_cap_usd` on `tenants` and `api_keys` (postgres-schema-v2.sql:25, 63) is stored and displayed (Keys & Tenants tab) but never enforced. The only rate-limit reference in the tree is a comment about *upstream* 429s (main.py:1426). No middleware, no 429 path, no limiter.

**Goal:** make tenant identity real on the request path and enforce spend/rate limits per tenant and per key, **without breaking the drop-in contract** ("change one `base_url`, keep the same key" — product-spec.md §1). Self-host default behavior must be byte-for-byte unchanged when the feature is off.
...
- The proxy-facing key is presented in a **new request header `X-Proxy-Key`** carrying the `tsk_…` plaintext exactly as minted (`_new_proxy_key`, main.py:1744-1748).
- **`Authorization` and `x-api-key` remain exclusively the upstream BYOK credential.** Reusing either would (a) collide with the x-api-key auth-style adapters (providers table, postgres-schema-v2.sql:38-51) and (b) force self-host users to choose between authenticating to us and authenticating upstream. The dedicated header avoids both; it is also invisible to any OpenAI SDK, which is why the default mode below must not require it.
...
- **Single mode switch `PROXY_AUTH_MODE` (Settings field, env `PROXY_AUTH_MODE`):**
  ...
  - `required`: `/v1/*` data-plane requests (`/v1/chat/completions`, `/v1/embeddings`, `/v1/models`) must carry a valid active key; **missing, unknown, revoked, or rotated → 401**, the compact error shape (`_unauthorized`, main.py:196-199). Fail-closed, always — an absent key must never fall through to the default tenant.
...
- When a request authenticates (required mode), the authenticating `tenant_id` **and** `api_key_id` are threaded into the ledger insert (stats.py:249-271 replaces its hardcoded zero-UUID literal with the resolved identity; `api_key_id` gets its first writer — the column exists, postgres-schema-v2.sql:80, and has none today).
- Exact-prefix cache lookup/record (caching.py:57-105) and semantic-cache scope construction (main.py:843) use the resolved `tenant_id` instead of `DEFAULT_TENANT_ID`. `cache_entries`'s unique key already includes `tenant_id` (postgres-schema-v2.sql:155), so **cross-tenant cache sharing stays structurally impossible**; per-tenant rows just stop all collapsing into `default`.
...
- No changes to `/health`, `/dashboard`, `/metrics`, `/api/kpis`, or the key-management endpoints' own auth (they keep the `ADMIN_TOKEN` bearer gate, `_require_admin`, main.py:201-212).
...
### D8. Observability
- New Prometheus counters on the existing `/metrics` surface (PlainTextResponse, main.py:1679): `token_saver_auth_failures_total{reason}`, `token_saver_quota_blocked_total{scope}`, `token_saver_rate_limited_total{scope}`, `token_saver_quota_check_unknown_total`.
...
- **AC-TQ2. `open` mode is byte-identical.** With `PROXY_AUTH_MODE` unset (default), a request with no `X-Proxy-Key` behaves exactly as at the pre-change tip fb6d849d (the post-gate baseline this implementation branches from): no auth check, ledger row on the default tenant, cache keys on the default tenant. Regression test: golden compare of ledger row + cache row against pre-change fixtures.
...
- **AC-TQ6. Spend-cap enforcement (in `required` mode).** Quota evaluation runs only when identity resolves, i.e. `PROXY_AUTH_MODE=required` — in `open` mode all traffic is the default tenant and quotas are unenforced (per D6). In `required` mode: tenant at/over its calendar-month `SUM(est_cost_after)` cap → 429 `quota_exceeded` before upstream forward; key-level cap overrides tenant cap when set; uncapped (both NULL) never blocks. Regression test (run in `required` mode): seeded month-to-date spend at cap−ε (pass), cap+ε (429), key-overrides-tenant, NULL-never-blocks; plus one `open`-mode assertion that no quota 429 is ever emitted.
- **AC-TQ7. Rate limits (in `required` mode).** Rate limiting is active only when `PROXY_AUTH_MODE=required`; in `open` mode no request is rate-limited (per D6). In `required` mode: sustained traffic over `RATE_LIMIT_KEY_RPM` → 429 with `Retry-After` for that key while other keys of the same tenant continue; tenant-aggregate limiter fires at `RATE_LIMIT_TENANT_RPM`. Regression test (run in `required` mode): bucket-filling requests with frozen clock; plus one `open`-mode assertion that sustained traffic never 429s.
...
- **AC-TQ11. Env manifest.** New vars (`PROXY_AUTH_MODE`, `QUOTA_FAIL_MODE`, `RATE_LIMIT_KEY_RPM`, `RATE_LIMIT_TENANT_RPM`) documented in README's env-var section (§"All settings are env vars", README:122) and appended with commented defaults to the existing `token-saver/.env.example` (verified present at fb6d849d — earlier draft said "absent"; that was wrong, the file lives under `token-saver/` not the repo root). The file is referenced by README at lines 26, 68, 122, and 216 (quickstart copy, POSTGRES_PASSWORD note, settings index, password-interpolation note); the added entries must be consistent with all four consumers (e.g. keep the `${POSTGRES_PASSWORD}`-style interpolation convention where applicable and the "NEVER commit the real .env" header).
- **AC-TQ12. Key lifecycle end-to-end.** Mint via `POST /api/keys` → use on data plane in `required` mode → rotate → old key 401s immediately, new key works (the rotate contract already promises "old key stops authenticating immediately", keys-tenants-tab-spec.md §3 API contract, :64 — this is the first scope where that promise is actually testable on the request path). Regression test: full lifecycle against a live app instance.
