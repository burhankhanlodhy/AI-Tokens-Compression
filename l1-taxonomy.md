...
Within an eligible JSON object, **drop by exact field name** any of these (present anywhere in the object graph):

| Field name(s) | Rationale |
|---|---|
| `score`, `relevance`, `relevance_score`, `similarity`, `distance`, `cosine`, `rerank_score` | retrieval plumbing, no answer-bearing signal |
| `chunk_id`, `chunk_index`, `block_id`, `passage_id`, `doc_id`, `source_id`, `id` (only when sibling `content`/`text` exists) | addressability / provenance, not content |
| `page`, `page_number`, `offset`, `length`, `char_start`, `char_end`, `token_start` | locators |
| `title`, `source`, `url`, `path`, `filename`, `collection`, `bucket`, `index_name` | provenance (value not required to answer) |
| `timestamp`, `created_at`, `updated_at`, `modified_at`, `expires_at`, `retrieved_at` | bookkeeping |
| `embedding`, `vector`, `vector_score`, `all(...)  ` float-array fields >32 elements | not human-readable content |
| `metadata`, `attributes`, `tags`, `custom`, `raw` (only when object-valued and all leaf values are in the dead list or strings matching the above) | nested dead wrappers |
| `retrieval`, `pagination`, `total`, `limit`, `offset`, `page`, `next_page`, `has_more`, `request_id`, `session_id`, `generation`, `query_id`, `trace` | harness / query plumbing |
| Empty strings, empty arrays, empty objects anywhere in an eligible JSON object | no signal |

`content` / `text` / `answer` / `passage` / `doc` fields whose value is the actual answer-bearing text are **reserved — never stripped**, even under a `metadata`-like parent.
...
- Round-trip equivalence definition for B4: for each fixture, `bounds(answer(clean(p)))` semantics equal `answer(p)` is replaced by the **structural equivalence** check: clean(p) preserves every reserved field (`content`/`text`/`passage`/`answer`/`role`/prose) byte-for-byte, and removes only permitted fields/whitespace per §4. No judge.
...
*Version 1.0 — frozen by @product-manager for B2/B3/B4/B5. Amendments require an audit note, not a silent edit.*